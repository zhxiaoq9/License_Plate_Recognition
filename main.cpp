...
int main(int argc, char** argv)
{
	Mat  src_gray;
	//Read image and convert it to gray image if it is RGB
	src_img = imread("./picture/car1.bmp");
	if (!src_img.data) { printf("Image read failure!\n"); return -1; }
...
//---------------------------step1 pre process---------------------------//
	...
	//namedWindow("Gauss Blur", 1);
	//imshow("Gauss Blur", src_gray);

	///Histogram equalization
	//equalizeHist(src_gray, src_gray);
	//namedWindow("equalization", 1);
	//imshow("equalization", src_gray);

	///gray level line transation
	Line_Trans(src_gray, src_gray);
	//namedWindow("line transation", 1);
	//imshow("line transation", src_gray);

	///convert to binary
	//threshold(src_gray, src_gray, 300, 255, THRESH_OTSU);
	//namedWindow("thresh_otsu", 1);
	//imshow("thresh_otsu", src_gray);

	///Canny Detect
	Canny(src_gray, src_gray,40,60);
	namedWindow("Canny", 1);
	imshow("Canny", src_gray);
	...
	//morphologyEx(src_gray, src_gray, CV_MOP_CLOSE, element);
	//dilate(src_gray, src_gray, element);
	//namedWindow("morphology", 1);
	//imshow("morphology", src_gray);

	//---------------------------step2---------------------------//
	Rect_Detect(src_gray, src_gray);
...
string train_file = "image_train.txt";
string test_file = "image_test.txt";
MyAnnTrain(train_file, test_file);
